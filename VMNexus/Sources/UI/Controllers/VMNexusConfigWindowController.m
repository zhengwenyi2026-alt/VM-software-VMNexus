//
//  VMNexusConfigWindowController.m
//  VMNexus
//
...
@interface VMNexusConfigWindowController () <NSTableViewDataSource, NSTableViewDelegate>
...
@implementation VMNexusConfigWindowController
...
- (instancetype)initWithVM:(VMNexusVirtualMachine *)vm isNew:(BOOL)isNew {
    ...
    if (self) {
        ...
        VMNexusArchitectureManager *mgr = [VMNexusArchitectureManager sharedManager];
        NSArray *profiles = [mgr allProfiles];
        if (profiles.count > 0) {
            VMNexusArchitectureProfile *firstProfile = profiles.firstObject;
...
- (void)setupUI {
    ...
    self.modeSwitcher = [[VMNexusModeSwitcher alloc] initWithFrame:NSMakeRect(8, self.stepSidebar.bounds.size.height - 36, 164, 26)];
...
- (NSView *)buildOverviewStep {
    ...
    for (NSString *osName in [VMNexusVirtualMachine allOSTypeNames]) {
...
- (NSView *)buildHardwareStep {
    ...
    [self.memoryUnitPopup addItemsWithTitles:[VMNexusVirtualMachine supportedUnits]];
...
- (NSView *)buildStorageStep {
    ...
    NSMutableArray *diskUnits = [[VMNexusVirtualMachine supportedUnits] mutableCopy];
...
- (void)updateNetworkStepForMode {
    BOOL isAdvanced = (self.appMode == VMNexusAppModeAdvanced);
    BOOL isSimple = (self.appMode == VMNexusAppModeSimple);
...
- (void)refreshSummary {
    ...
    VMNexusEngineType engineType = [VMNexusEngineSelector engineTypeForMachine:machine];
    NSString *engineName = [VMNexusEngineSelector nameForEngineType:engineType];
...
- (void)archChanged:(id)sender {
    VMNexusArchitectureProfile *profile = self.archPopup.selectedItem.representedObject;
    if (!profile) return;
...
- (void)osTypeChanged:(id)sender {
    ...
    VMNexusOSType osType = (VMNexusOSType)self.osPopup.indexOfSelectedItem;
    NSDictionary *preset = [VMNexusVirtualMachine presetConfigForOSType:osType];
    if (!preset) return;
    // Apply architecture
...
- (NSDictionary *)engineUILimitsForType:(VMNexusEngineType)type {
    switch (type) {
        ...
        case VMNexusEngineTypeFSUAE:
            return @{
                ...
                @"networkAvailable": @NO, @"usbAvailable": @NO, @"hvfAvailable": @NO,
                @"isoAvailable": @NO, @"cdromAvailable": (type == VMNexusEngineTypeFSUAE ? @YES : @NO),
...
- (void)updateUIForCurrentMachine {
    ...
    if (memoryFixed) {
        ...
        NSInteger currentMB = [VMNexusVirtualMachine convertValue:self.memoryField.integerValue
...
- (void)saveConfig:(id)sender {
    ...
    self.virtualMachine.osType = (VMNexusOSType)self.osPopup.indexOfSelectedItem;
    NSString *machine = self.machinePopup.titleOfSelectedItem ?: @"default";
    ...
    if (memoryFixed) {
        ...
        NSInteger memMB = [VMNexusVirtualMachine convertValue:memVal fromUnit:memUnit toUnit:@"MB"];
    ...
    if (self.isNewVM) {
        NSString *dir = [[NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES) firstObject]
                         stringByAppendingPathComponent:@"VMNexus/VirtualMachines"];
        NSString *vmPath = [dir stringByAppendingPathComponent:
                           [NSString stringWithFormat:@"%@.vmnexus", self.virtualMachine.vmIdentifier]];
