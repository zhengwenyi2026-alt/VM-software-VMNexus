//
//  VMNexusConsoleWindowController.m
//  VMNexus
//
...
@property (nonatomic, strong) VMNexusVNCViewer *vncViewer;
@property (nonatomic, strong) VMNexusSerialTerminal *serialTerminal;
@property (nonatomic, strong) VMNexusResourceMonitor *resourceMonitor;
...
@implementation VMNexusConsoleWindowController
...
- (instancetype)initWithVM:(VMNexusVirtualMachine *)vm {
    ...
    window.title = [NSString stringWithFormat:@"%@ - VMNexus", vm.name];
    ...
    if (self) {
        _virtualMachine = vm;
        _engine = [VMNexusEngineSelector engineForVM:vm];
        _engine.delegate = self;
        if ([_engine isKindOfClass:[VMNexusQEMUProcess class]]) {
            _qemuProcess = (VMNexusQEMUProcess *)_engine;
...
- (void)setupUI {
    ...
    self.vncViewer = [[VMNexusVNCViewer alloc] initWithFrame:self.displayContainer.bounds];
    ...
    self.serialTerminal = [[VMNexusSerialTerminal alloc] initWithFrame:self.serialContainer.bounds];
    ...
    self.resourceMonitor = [[VMNexusResourceMonitor alloc] initWithFrame:self.monitorContainer.bounds];
    ...
    [self appendLog:[NSString stringWithFormat:@"VMNexus Console - %@\nArchitecture: %@\n\n",
...
- (void)setupSplashView {
    ...
    self.splashTitle.stringValue = @"VMNexus";
...
- (void)togglePauseResume:(id)sender {
    if (self.virtualMachine.status == VMNexusStatusPaused) {
...
- (void)takeScreenshot:(id)sender {
    ...
    if (captured) {
        ...
        NSString *filename = [NSString stringWithFormat:@"VMNexus_%@.png", timestamp ?: @"screenshot"];
        ...
        NSString *path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"vmnexus_screenshot.ppm"];
...
- (void)stopRecording {
    ...
    if (frameCount > 0) {
        ...
        NSString *dirPath = [[[NSSearchPathForDirectoriesInDomains(NSDesktopDirectory, NSUserDomainMask, YES) firstObject]
                             stringByAppendingPathComponent:[NSString stringWithFormat:@"VMNexus_Recording_%@", timestamp ?: @"rec"]]
...
- (void)showMediaMenu:(id)sender {
    ...
    NSArray *opticalDrives = [VMNexusVirtualMachine physicalOpticalDrives];
    ...
    NSArray *floppyDrives = [VMNexusVirtualMachine physicalFloppyDrives];
...
#pragma mark - VMNexusQEMUProcessDelegate
...
#pragma mark - VMNexusEngineDelegate
