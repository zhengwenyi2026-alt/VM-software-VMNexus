//
//  VMNexusFuseProcess.m
//  VMNexus
//
//  Fuse ZX Spectrum engine wrapper.
...
@interface VMNexusFuseProcess ()
@property (nonatomic, strong, readwrite) VMNexusVirtualMachine *virtualMachine;
@property (nonatomic, assign, readwrite) BOOL isRunning;
@property (nonatomic, strong) NSTask *task;
@end

@implementation VMNexusFuseProcess

@synthesize delegate;

- (VMNexusEngineType)engineType { return VMNexusEngineTypeFuse; }
...
- (BOOL)startWithError:(NSError **)error {
    NSString *binPath = [VMNexusEngineSelector executablePathForEngineType:VMNexusEngineTypeFuse];
    if (!binPath) {
        if (error) *error = [NSError errorWithDomain:@"VMNexus" code:1001
            userInfo:@{NSLocalizedDescriptionKey: @"Fuse not found. Please install Fuse."}];
        return NO;
    }
