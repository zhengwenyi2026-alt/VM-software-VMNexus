//
//  VMNexusFSUAEProcess.m
//  VMNexus
//
//  FS-UAE Amiga engine wrapper.
...
@interface VMNexusFSUAEProcess ()
@property (nonatomic, strong, readwrite) VMNexusVirtualMachine *virtualMachine;
@property (nonatomic, assign, readwrite) BOOL isRunning;
@property (nonatomic, strong) NSTask *task;
@end

@implementation VMNexusFSUAEProcess

@synthesize delegate;

- (VMNexusEngineType)engineType { return VMNexusEngineTypeFSUAE; }
...
- (BOOL)startWithError:(NSError **)error {
    NSString *binPath = [VMNexusEngineSelector executablePathForEngineType:VMNexusEngineTypeFSUAE];
    if (!binPath) {
        if (error) *error = [NSError errorWithDomain:@"VMNexus" code:1001
            userInfo:@{NSLocalizedDescriptionKey: @"FS-UAE not found. Please install FS-UAE."}];
        return NO;
    }
    ...
    if (self.virtualMachine.floppyAPath.length > 0) {
        [args addObjectsFromArray:@[@"--floppy_drive_0", self.virtualMachine.floppyAPath]];
    }
    if (self.virtualMachine.diskImagePath.length > 0) {
        [args addObjectsFromArray:@[@"--hard_drive_0", self.virtualMachine.diskImagePath]];
    }
