...
@interface VMNexusAppDelegate ()
@property (nonatomic, strong) VMNexusLibraryWindowController *libraryWC;
@end

@implementation VMNexusAppDelegate

- (void)applicationDidFinishLaunching:(NSNotification *)notification {
    if (![[VMNexusArchitectureManager sharedManager] isQEMUInstalled]) {
    ...
    [self setupMainMenu];
    self.libraryWC = [[VMNexusLibraryWindowController alloc] init];
...
- (void)showHelp:(id)sender {
    [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:@"https://github.com/zhengwenyi2026-alt/VMNexus"]];
}

- (void)autoStartVMs {
    for (VMNexusVirtualMachine *vm in self.libraryWC.virtualMachines) {
        if (vm.autoStart && vm.status != VMNexusStatusRunning) {
...
- (void)openDiskManager:(id)sender {
    ...
    [panel beginSheetModalForWindow:self.libraryWC.window completionHandler:^(NSInteger result) {
        if (result == NSModalResponseOK) {
            VMNexusDiskManagerController *diskMgr = [[VMNexusDiskManagerController alloc] initWithDiskPath:panel.URL.path];
