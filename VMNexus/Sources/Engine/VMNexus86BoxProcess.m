...
@interface VMNexus86BoxProcess ()
@property (nonatomic, strong, readwrite) VMNexusVirtualMachine *virtualMachine;
@property (nonatomic, assign, readwrite) BOOL isRunning;
@property (nonatomic, strong) NSTask *task;
@end

@implementation VMNexus86BoxProcess

@synthesize delegate;

- (VMNexusEngineType)engineType { return VMNexusEngineType86Box; }
...
- (BOOL)startWithError:(NSError **)error {
    NSString *binPath = [VMNexusEngineSelector executablePathForEngineType:VMNexusEngineType86Box];
    if (!binPath) {
