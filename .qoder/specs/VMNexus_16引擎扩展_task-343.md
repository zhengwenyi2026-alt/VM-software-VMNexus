...
为每个引擎分配一个**机器类型前缀**，`VMNexusEngineSelector` 按前缀 + 显式集合自动路由：
...
> 注：前缀规则放在 `VMNexusEngineSelector` 的一张 NSDictionary 里，便于后续新增引擎时只改一处。

## 三、代码改造范围

### 3.1 协议与枚举
- `VMNexusEngineProtocol.h`：`VMNexusEngineType` 扩展为 16 个枚举值。
- 新增通用发现方法到 `VMNexusEngineSelector`：
  - `+engineTypeForMachine:`
  - `+executablePathForEngineType:`
  - `+allEngineTypes`

### 3.2 新增 13 个引擎 wrapper
每个 wrapper 一个 `.h/.m` 文件，统一实现 `VMNexusEngine` 协议：
- `VMNexusSheepShaverProcess`
- `VMNexusDOSBoxProcess`
- `VMNexusDOSBoxXProcess`
- `VMNexusPCemProcess`
- `VMNexus86BoxProcess`
- `VMNexusVICEProcess`
- `VMNexusFSUAEProcess`
- `VMNexusHatariProcess`
- `VMNexusOpenMSXProcess`
- `VMNexusFuseProcess`
- `VMNexusScummVMProcess`
- `VMNexusMednafenProcess`
- `VMNexusARAnyMProcess`
...
### 3.3 机型列表
- `VMNexusArchitectureManager.m`：扩展 `m68k` 机器列表，并新增 `i386`、`powerpc`、`m68k-atari`、`z80`、`mos6502` 等架构条目，把对应机器类型合并进去。
- 例如：
...
### 3.4 UI 自适应
重构 `VMNexusConfigWindowController`：
...
- `VMNexus/Sources/Engine/VMNexusEngineProtocol.h`
- `VMNexus/Sources/Engine/VMNexusEngineSelector.h/.m`
- `VMNexus/Sources/Engine/VMNexusQEMUProcess.h/.m`（可能需小改兼容）
- 新增 13 对 `.h/.m` wrapper 文件
- `VMNexus/Sources/Models/VMNexusArchitectureManager.m`
- `VMNexus/Sources/UI/Controllers/VMNexusConfigWindowController.m`
- `VMNexus/Resources/zh-Hans.lproj/Localizable.strings`
- `VMNexus/Resources/en.lproj/Localizable.strings`
- `VMNexus.xcodeproj/project.pbxproj`