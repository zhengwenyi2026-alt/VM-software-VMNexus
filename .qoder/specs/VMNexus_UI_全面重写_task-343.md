...
## 1. Library 主窗口重写 (VMNexusLibraryWindowController.m)
...
## 2. Console 控制台窗口重写 (VMNexusConsoleWindowController.m)
...
### VMNexus 启动屏
- 全屏深色背景
- 居中显示 "VMNexus" 大字 (48pt SF Pro Display Bold)
...
## 3. Config Wizard 配置向导重写 (VMNexusConfigWindowController.m)
...
### 自定义视图类
- `VMNexusCardView`: 圆角卡片 (背景色 + 阴影 + 圆角)
- `VMNexusIconButton`: 带图标的按钮 (hover 效果)
- `VMNexusStatusBadge`: 状态徽章 (圆点 + 文字)
...
1. 创建通用组件类 (VMNexusCardView, VMNexusIconButton, VMNexusStatusBadge)
2. 重写 Library 窗口 UI
...
| 文件 | 操作 |
|------|------|
| `VMNexus/Sources/UI/Views/VMNexusCardView.h/m` | 新增 |
| `VMNexus/Sources/UI/Views/VMNexusIconButton.h/m` | 新增 |
| `VMNexus/Sources/UI/Views/VMNexusStatusBadge.h/m` | 新增 |
| `VMNexus/Sources/UI/Controllers/VMNexusLibraryWindowController.m` | 重写 |
| `VMNexus/Sources/UI/Controllers/VMNexusConsoleWindowController.m` | 重写 |
| `VMNexus/Sources/UI/Controllers/VMNexusConfigWindowController.m` | 重写 |
| `VMNexus/Resources/en.lproj/Localizable.strings` | 更新 |
| `VMNexus/Resources/zh-Hans.lproj/Localizable.strings` | 更新 |
