//
//  VMStartDOSBoxProcess.m
//  VMStart
//
//  DOSBox MS-DOS engine wrapper.
...
@interface VMStartDOSBoxProcess ()
@property (nonatomic, strong, readwrite) VMStartVirtualMachine *virtualMachine;
@property (nonatomic, assign, readwrite) BOOL isRunning;
@property (nonatomic, strong) NSTask *task;
@end

@implementation VMStartDOSBoxProcess

@synthesize delegate;

- (VMStartEngineType)engineType { return VMStartEngineTypeDOSBox; }
...
- (BOOL)startWithError:(NSError **)error {
    NSString *binPath = [VMStartEngineSelector executablePathForEngineType:VMStartEngineTypeDOSBox];
    if (!binPath) {
        if (error) *error = [NSError errorWithDomain:@"VMStart" code:1001
            userInfo:@{NSLocalizedDescriptionKey: @"DOSBox not found. Please install DOSBox."}];
        return NO;
    }
