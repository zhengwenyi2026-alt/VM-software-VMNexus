...
@interface VMStartMiniVMacProcess ()
@property (nonatomic, strong, readwrite) VMStartVirtualMachine *virtualMachine;
@property (nonatomic, assign, readwrite) BOOL isRunning;
@property (nonatomic, strong) NSTask *task;
@end

@implementation VMStartMiniVMacProcess

@synthesize delegate;

- (VMStartEngineType)engineType { return VMStartEngineTypeMiniVMac; }
...
- (BOOL)startWithError:(NSError **)error {
    ...
    NSString *binPath = [VMStartEngineSelector executablePathForEngineType:VMStartEngineTypeMiniVMac];

    if (self.virtualMachine.romFilePath.length == 0) {
        if (error) {
            *error = [NSError errorWithDomain:@"VMStart" code:2002
...
- (BOOL)startViaWorkspaceWithError:(NSError **)error {
    ...
    if (!appPath) {
        if (error) {
            *error = [NSError errorWithDomain:@"VMStart" code:2001
