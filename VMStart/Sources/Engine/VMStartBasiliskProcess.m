...
@interface VMStartBasiliskProcess ()
...
@implementation VMStartBasiliskProcess

@synthesize delegate;

- (VMStartEngineType)engineType { return VMStartEngineTypeBasiliskII; }
...
- (BOOL)startWithError:(NSError **)error {
    NSString *binPath = [VMStartEngineSelector executablePathForEngineType:VMStartEngineTypeBasiliskII];
    if (!binPath) {
        if (error) {
            *error = [NSError errorWithDomain:@"VMStart" code:1001
    ...
    if (self.virtualMachine.romFilePath.length == 0) {
        if (error) {
            *error = [NSError errorWithDomain:@"VMStart" code:1002
    ...
    if (!prefsPath) {
        if (error) {
            *error = [NSError errorWithDomain:@"VMStart" code:1003
...
- (NSString *)writePrefsFile {
    VMStartVirtualMachine *vm = self.virtualMachine;
    ...
    NSInteger memMB = [VMStartVirtualMachine convertValue:vm.memoryValue
