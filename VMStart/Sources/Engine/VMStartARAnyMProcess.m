//
//  VMStartARAnyMProcess.m
//  VMStart
//
//  ARAnyM Atari VM engine wrapper.
...
@interface VMStartARAnyMProcess ()
@property (nonatomic, strong, readwrite) VMStartVirtualMachine *virtualMachine;
@property (nonatomic, assign, readwrite) BOOL isRunning;
@property (nonatomic, strong) NSTask *task;
@end

@implementation VMStartARAnyMProcess

@synthesize delegate;

- (VMStartEngineType)engineType { return VMStartEngineTypeARAnyM; }
...
- (BOOL)startWithError:(NSError **)error {
    NSString *binPath = [VMStartEngineSelector executablePathForEngineType:VMStartEngineTypeARAnyM];
    if (!binPath) {
        if (error) *error = [NSError errorWithDomain:@"VMStart" code:1001
            userInfo:@{NSLocalizedDescriptionKey: @"ARAnyM not found. Please install ARAnyM."}];
        return NO;
    }
    ...
    if (self.virtualMachine.diskImagePath.length > 0) {
        [args addObjectsFromArray:@[@"--disk", self.virtualMachine.diskImagePath]];
    }
